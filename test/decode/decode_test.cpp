...
struct c_1
{
	...
	json encode() const
	{
		...
		result.push_back(a);
		result.push_back(spiritsaway::serialize::encode(b));
		result.push_back(spiritsaway::serialize::encode(c));
		return result;
	}
	bool decode(const json& data)
	{
		...
		if (!spiritsaway::serialize::decode(data[0], a))
		{
			return false;
		}
		if (!spiritsaway::serialize::decode(data[1], b))
		{
			return false;
		}
		if (!spiritsaway::serialize::decode(data[2], c))
		{
			return false;
		}
		return true;
